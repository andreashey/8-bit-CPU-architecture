...
create_clock -name clk_100m -period 10.00 -waveform {0 5} [get_ports { clk_100m }];

# Arrival time = clock edge (10ns) - output delay (4ns) = 6 ns 
# Set input and output delay
# External device latches output between 1 ns and 7 ns after clock edge
#set_output_delay -clock clk_100m -max -4.0 [get_ports o_output] ;# Output must be valid by 1 ns
#set_output_delay -clock clk_100m -min 1.0 [get_ports o_output] ;# Output must remain stable until 7 ns

set_false_path -from [get_ports rst_btn_n]

#set_input_delay -clock clk_100m -max 10.0 [get_ports rst_btn_n]
#set_input_delay -clock clk_100m -min 0.0 [get_ports rst_btn_n]

#ignore timing
#set_false_path -from [get_ports o_output]
#set_false_path -from [get_ports rst_btn_n]
